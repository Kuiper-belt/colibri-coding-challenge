...
def _merge_paths_with_base(config: dict) -> None:
    ...
    # Merge path in bronze_layer_config
    if "bronze_layer_config" in etl_cfg and "write_options" in etl_cfg["bronze_layer_config"]:
        short_path = etl_cfg["bronze_layer_config"]["write_options"].get("path")
        if short_path:
            etl_cfg["bronze_layer_config"]["write_options"]["path"] = merge_path(short_path)
            logger.debug(f"Merged bronze_layer_config path: {short_path} -> {etl_cfg['bronze_layer_config']['write_options']['path']}")